...
B127 <- full_join(B127.MET, B127.HB)
...
B127.mod <- B127.convert %>% mutate(Time5 = ifelse(is.na(Time5_A), as.character(Time5_B), as.character(Time5_A)),
                                    ...
                                    Soil_Temp = ifelse(is.na(Soil_Temp_MET), Soil_Temp_Z, Soil_Temp_MET),
                                    Soil_Moisture = ifelse(is.na(Soil_Moisture_MET), Soil_Moisture_A, Soil_Moisture_MET),
                                    Air_Temp = ifelse(is.na(Air_Temp_MET), Air_Temp_Z, Air_Temp_MET))
Plot.title <- "B127"
